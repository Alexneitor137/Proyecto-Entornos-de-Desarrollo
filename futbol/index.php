...
<script>
const PRECIO=50;
let reservas = <?php echo json_encode($reservas_js); ?>;
...
let div=document.querySelector('#horas');
div.innerHTML='';
for(let h=9;h<25;h++){
let hora=(h%24).toString().padStart(2,'0')+':00';
let inicio=new Date(fecha+' '+hora).getTime()/1000;
let dur=parseInt(document.querySelector('[name="duracion"]').value);
let fin=inicio+dur*3600;
let ocupada=false;
let libre='';

reservas.forEach(r=>{
if(r.campo==campo){
let r_inicio=new Date(r.fecha+' '+r.hora).getTime()/1000;
let r_fin=new Date(r.fecha+' '+r.fin).getTime()/1000;
...
b.type='button';
b.innerText=hora;
if(ocupada){b.style.background='#400';b.style.color='#ff4444';b.title='Ocupado hasta '+libre;b.disabled=true;}
else{b.style.background='#050614';b.style.color='#00ff00';}
b.onclick=()=>document.querySelector('#hora').value=hora;
div.appendChild(b);
}
}
</script>
